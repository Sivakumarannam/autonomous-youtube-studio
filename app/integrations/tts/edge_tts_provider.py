...
class EdgeTTSProvider:
    def __init__(self, voice: str = "en-US-JennyNeural", sample_rate: int = 22050):
        self.voice = voice
    ...
    async def synthesize_speech(self, text: str, output_path: str) -> str:
        ...
        await asyncio.to_thread(self._write_wave, output_file, text, self.sample_rate, 220)
        return str(output_file)

    def _write_wave(self, path: Path, text: str, sample_rate: int, frequency: int) -> None:
        duration_secs = max(1.0, min(10.0, len(text) * 0.04))
