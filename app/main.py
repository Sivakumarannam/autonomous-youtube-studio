...
def create_app() -> FastAPI:
    ...
    async def authentication_handler(request: Request, exp: AuthenticationError) -> JSONResponse:
        return JSONResponse(status_code=401, content={"error": exp.code, "message": exp.message})

    @app.exception_handler(AuthorizationError)
    async def authorization_handler(request: Request, exp: AuthorizationError) -> JSONResponse:
        return JSONResponse(status_code=403, content={"error": exp.code, "message": exp.message})

    @app.exception_handler(PipelineError)
    async def pipeline_error_handler(request: Request, exp: PipelineError) -> JSONResponse:
        return JSONResponse(status_code=422, content={"error": exp.code, "message": exp.message})

    @app.exception_handler(PublishError)
    async def publish_error_handler(request: Request, exp: PublishError) -> JSONResponse:
